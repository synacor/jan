...
function buildNav(members, opts) {
    ...
    nav += '<h2><a href="index.html">' + (opts.indexName || 'Index') + '</a></h2>';

    for (var name in list) {
        if (members[list[name]].length) {
            nav += '<div class="nav-'+list[name]+'">';
            nav += '<h3>'+name+'</h3><ul>';
            members[list[name]].forEach(function(m) {
                ...
                var funcs = find({
                    kind : 'function',
                    memberof : m.longname==='Global' ? {isUndefined:true} : m.longname
                });

                if (funcs.length) {
                    nav += '<div class="navinner nav-methods"><h4>Methods</h4><ul>';
                    funcs.forEach(function(f) {
                        if (!hasOwnProp.call(seen, f.longname)) {
                            nav += '<li title="'+f.longname+'">'+linkto(f.longname, f.name)+'</li>';
                        }
                        seen[m.longname] = true;
                    });
                    nav += '</ul></div>';
    ...
    if (members.globals.length) {
        members.globals.forEach(function(g) {
            if ( g.kind !== 'typedef' && !hasOwnProp.call(seen, g.longname) ) {
                globalNav += '<li title="'+g.longname+'">' + linkto(g.longname, g.name) + '</li>';
            }
